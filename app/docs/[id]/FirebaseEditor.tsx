...
export default function FirebaseEditor({ docId }: { docId: string }) {
	...
	useEffect(() => {
		...
		const setup = async () => {
			...
			const quill = new (Quill as any)(editorEl, {
				theme: "snow",
				modules: {
					toolbar: [
						[{ header: [1, 2, 3, false] }],
			...
			unsubDoc = onSnapshot(docRef, (ds) => {
				...
				try {
					...
					if (JSON.stringify(current?.ops) !== JSON.stringify(incoming.ops)) {
						quillRef.current?.setContents(incoming);
					}
				} catch {}
			});
			...
			quill.on("text-change", (_delta: any, _old: any, source: string) => {
				...
				savingTimer.current = window.setTimeout(async () => {
					try {
						...
						await setDoc(
							docRef,
							{ deltaOps: delta.ops, updatedAt: serverTimestamp() } as FirestoreDeltaDoc,
							{ merge: true },
						);
			...
			const onApplyDelta = (e: Event) => {
				...
				try {
					...
					void setDoc(
						docRef,
						{ deltaOps: ops, updatedAt: serverTimestamp() } as FirestoreDeltaDoc,
						{ merge: true },
					);
				} catch (error) {
			...
			unsubPresence = onSnapshot(presenceCol, (qs) => {
				...
				qs.docChanges().forEach((change) => {
					...
					if (data.index < 0) {
						try { cursors.removeCursor(id); } catch {}
						return;
					}
					try {
						...
						cursors.moveCursor(id, { index: data.index, length: data.length ?? 0 });
					} catch {}
				});
			});
